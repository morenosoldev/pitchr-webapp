  ...
  io.on("connection", (socket) => {
    socket.on("join", async (user) => {
      ...
      if (users.has(user.user_id)) {
        ...
        existingUser.sockets = [...existingUser.sockets];
        users.set(user.user_id, existingUser);
        sockets = [...existingUser.sockets];
        userSockets.set(socket.id, user.user_id);
      } else {
    ...
    socket.on("message", async (message) => {
      ...
      try {
        const msg = {
          type: message.type,
          fromUserId: message.fromUser.user_id,
        ...
        message.fromUserId = message.fromUser.user_id;
      ...
      `
        ...
        inner join (
            ...
            where exists (
                ...
                where u.id = ? and c.id = ChatUsers.chatId
            )
