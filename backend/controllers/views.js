...
const getPitchViews = async(req,res) => {
    ...
    try {        
    const pitchViews = await PitchViews.findAll({where: {
        user_id: id,
        createdAt: {
    ...
    const result = pitchViews.reduce((acc, current) => {
        ...
        const existing = acc.find(a => new Date(a.createdAt).toISOString().slice(0, 10) === new Date(createdAt).toISOString().slice(0, 10));
        if (existing) {
           existing.updatedAt = new Date(existing.createdAt).toDateString();
           existing.id = new Date(existing.createdAt).getDay() -1;
           existing.count += 1;
           var index = preFilled.findIndex(item => item.updatedAt == new Date(existing.createdAt).toDateString())
           if(index > 0){
           ...
           var index = preFilled.findIndex(item => item.updatedAt == new Date(createdAt).toDateString())
           if(index > 0){
        ...
        var index = result.map(function(e) { return e.updatedAt; }).indexOf('Nick');
/*
...
const getProfileViews = async(req,res) => {
    ...
    try {        
    const profileViews = await ProfileViews.findAll({where: {
        user_id: id,
        createdAt: {
        [Op.gte]: moment().subtract(7, 'days').toDate()
        }
...
module.exports = {addPitchView,addProfileView,getPitchViews,getProfileViews}