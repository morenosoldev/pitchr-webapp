...
import {Card,Form,Container,Row, ProgressBar} from 'react-bootstrap'
...
useEffect(async() => {
  ...
  const fetchProfileViews = async () => {
   const data = await ViewsService.getProfileViews(user.user_id)
    setProfileViewChart({
      labels: data?.map(view => view.updatedAt.split(" ")[1] + " " +view.updatedAt.split(" ")[2]),
      datasets: [
        {
          label: 'Profile Views',
...
const options = {
    ...
    Hey you haven't added your calendly account yet. <Alert.Link href="#" onClick={handleShow}>Click here</Alert.Link> to add it.
  </Alert> 
    )}
...
<Modal show={show} onHide={handleClose}>
  ...
  <Form.Group className="mb-3">
    ...
    <Form.Control onChange={(e) => setCalendly(e.target.value)} type="input" placeholder="Enter calendly url" />
      ...
      <Card.Body>
        <Row>
          <Col sm>
            ...
            <ProgressBar style={{marginTop:15}} now={progress} label={`${progress}%`}/>
          </Col>
    ...
    <Form.Check
        inline
        onClick={() => history.push(`/business/app/company/${user?.user_id}/about-us`)}
    ...
    <Form.Check
        ...
        onClick={() => history.push(`/business/app/company/${user?.user_id}/team`)}
        style={{paddingLeft:'8rem'}}
      />
    </Col>
    ...
    <Form.Check
        ...
        onClick={() => history.push(`/business/app/company/${user?.user_id}/files`)}
      />
    </Col>
...
<Card>
  <Card.Body>
      <h2> Welcome back, {user?.name}</h2>
            <h3 style={{marginBottom:20}}>Here is your daily investor activity overview</h3>
            <Row>
            <Col sm>
