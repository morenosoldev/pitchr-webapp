...
function Upload({addChart}) {
    ...
    <Form.Control type="text" value={metric} onChange={(e) => setMetric(e.target.value)} placeholder="Enter name" />
        ...
        <Col>
        <OutTable data={data} cols={cols} addChart={addChart} metric={metric}/>
        </Col>
        </Row>
	</DragDropFile>
	);
...
const user = useSelector(state => state.authentication.user);
  ...
  const addItem = (key) => {
	  console.log(activeHeader.length)
  ...
  const submitData = async() => {
	...
	const months = pattern.test(data[0][activeHeader[0]]) ? newArr.map(item => item[activeHeader[0]]): pattern.test(data[0][activeHeader[1]]) ? newArr.map(item => item[activeHeader[1]]): setShowError(true) 
	if(!months) fail = true;

	if(fail) return;
	
	const values = arrayEquals(months, newArr.map(item => item[activeHeader[1]])) ? newArr.map(item => item[activeHeader[0]]): newArr.map(item => item[activeHeader[1]]);

	console.log("Måneder", months);
...
const arr = newArr.map(function(item,i) { return months[i] !== undefined ? {"month": months[i],"amount":  values[i]}: null})  
...
<p>Choose the month and data field.</p>
	<button disabled={activeHeader.length == 2 ? false: true} onClick={submitData} className="btn btn-success">Upload</button>
	{showError ? (
		<Alert className='mt-5' variant="danger" onClose={() => setShowError(false)} dismissible>
    ...
    <div className="table-responsive mt-5">
            ...
            <table className="table table-striped">	
                <thead>
					<tr>{cols.map((c) => <th onClick={() => addItem(c.key)}    className={`columnHeader ${activeHeader?.some(r => r == c.key) ? "activeColumn" : ""}`} key={c.key}>{c.name}</th>)}</tr>
				</thead>

				<tbody>
					{data.map((r,i) => <tr key={i}>
						{cols.map(c => <td key={c.key}   className={`columnHeader ${activeHeader?.some(r => r == c.key) ? "activeColumn" : ""}`}>{ r[c.key] }</td>)}
					</tr>)}
				</tbody>
			</table>
