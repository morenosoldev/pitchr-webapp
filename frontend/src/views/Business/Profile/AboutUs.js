...
const submitDescription = () => {
 ...
 const submitDevolpmentStage = () => {
    if(developmentStage.length > 0){
      dispatch(userActions.updateDevelopmentStage(developmentStage, user?.user_id));
 ...
 const submitCompetences = async(e) => {
   ...
   await API.put(`/updateCompetences/${user?.user_id}`, {"data": payload}).then(res => {
...
<Form>
  ...
  <Button style={{marginLeft:10}} onClick={() => setWriteDescription(false)}>Cancel</Button>
</Form>
                                                </div>) :  
                                        <>
        ...
        <Card>
                <Card.Body className=" profile-page">
            <div className='profile-setting-header'>
            <h4 className="mb-3" style={{marginBottom:'0px', marginRight:'10px'}}>Development stage</h4>
            {edit && developmentStage?.length > 0 ? (
            <div className="user-img img-fluid"><i className="fas fa-user-edit" onClick={() => setWriteDevelopmentStage(true)} style={{cursor:'pointer'}}></i></div>
            ): null}
            </div>
...
<Form.Group className="mb-3">
      ...
      <Form.Control as="select" value={developmentStage} onChange={(e) => setDevelopmentStage(e.target.value)}>
  ...
  <Button onClick={() => submitDevolpmentStage()}>Submit</Button>
  <Button style={{marginLeft:10}} onClick={() => setWriteDevelopmentStage(false)}>Cancel</Button>
                                             ...
                                             {user?.id !== profile?.id ? (
                                                <>
                                                   ...
                                                   : 
                                                      <ul className="suggestions-lists m-0 p-0">
                                                        <li className="d-flex mb-4 align-items-center">
                                                         <div className="user-img img-fluid"><i className="ri-add-fill" onClick={() => setWriteDevelopmentStage(true)} style={{cursor:'pointer'}}></i></div>
                                                         <div className="ms-3">
...
<Form>
  ...
  <Button style={{marginLeft:10}} onClick={() => setWriteMarkets(false)}>Cancel</Button>
            ...
            <div className="user-img img-fluid"><i className="fas fa-user-edit" onClick={() => setWriteIndustry(true)} style={{cursor:'pointer'}}></i></div>                                                          :
             null}
             </div>
...
onChange={addIndustry}
  ...
  <Button style={{marginLeft:10}} onClick={() => setWriteIndustry(false)}>Cancel</Button>
         ...
         <Row>
            ...
            <Col sm> 
            <div className='profile-setting-header'>
            <p style={{marginBottom:'0px', marginRight:'10px'}}>Capital</p>
            {edit && goal?.length > 0 ? (
            <div className="user-img img-fluid"><i className="fas fa-user-edit" onClick={() => setWriteGoal(true)} style={{cursor:'pointer'}}></i></div>
            ): null}
            </div>
            {writeGoal ? (
             <div>
  ...
  <fieldset>
    ...
    <Form.Group className="mb-3">
      <Form.Label htmlFor="textInput">How much ownership are you offering?</Form.Label>
      <Form.Control id="textInput" placeholder="Percentage" as="input" onChange={(e) => setPercentage(e.target.value)}/>
    </Form.Group>

    <Button onClick={() => submitGoalAndPercentage()} type="submit">Submit</Button>
    <Button style={{marginLeft:10}} onClick={() => setWriteGoal(false)}>Cancel</Button>
  </fieldset>
</Form>
</div>
) :
                                             ...
                                             {user?.id !== profile?.id ? (
                                                <>
                                        <h6>{goal} for {percentage}% of our company.</h6>
                                                </>
                                                ):(
<>
{user?.goal ? 
                                                   ...
                                                   <li className="d-flex mb-4 align-items-center">
                                                      ...
                                                      <div className="ms-3">
                                                         <h6>{user?.goal} for {user?.percentage}% of our company.</h6>
                                                      </div>
                                                   </li>
                                                </ul>
                                                   : 
                                                      <ul className="suggestions-lists m-0 p-0">
                                                       <li className="d-flex mb-4 align-items-center">
                                                         <div className="user-img img-fluid"><i className="ri-add-fill" onClick={() => setWriteGoal(true)} style={{cursor:'pointer'}}></i></div>
                                                         <div className="ms-3">
            ...
            <p style={{marginBottom:'0px', marginRight:'10px'}}>Competences</p>  
         {edit && !writeCompetences && competences?.length > 0 ? (
         <div className="user-img img-fluid"><i className="fas fa-user-edit" onClick={() => setWriteCompetences(true)} style={{cursor:'pointer'}}></i></div>
         ): null}   
            </div>
...
<Badge style={{padding:'8px',margin:'6px'}} pill bg="primary">
{competence.name}
<i style={{paddingLeft:'4px',cursor:'pointer'}} class="fas fa-trash-alt" onClick={() => removeCompetence(key)}></i>
</Badge>
               ))}
      </ul>
    ): null}
