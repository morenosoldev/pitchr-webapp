...
export default function ModalOne({prevStep,nextStep,changeVideoFile,publishVideo,changeLoomHTML,publishLoomVideo,progress,setProgress,video,step}) {
...
const uploadVideo = async (video) => {
    ...
    await videoRef.put(video).on(
      ...
      () => {
        ...
        storage.ref(`videos/${video.name}`).getDownloadURL()
          .then(
            (url) => {
              // got download URL
             publishVideo(url);
...
<Row>
 ...
 <div style={{display:'flex',flexDirection:'column', alignItems:'center', justifyContent:'center',marginBottom:15}}>
            ...
            <p style={{marginTop:25}}>Drag and drop your pre recorded pitch!</p>
    ...
    <Form.Control type="file" onChange={(e) => next(e)}/>
         ...
         <Col sm>
         <div style={{display:'flex',flexDirection:'column', alignItems:'center', justifyContent:'center',marginBottom:15}}>
            <i style={{fontSize:'4rem', padding:40, borderRadius:'50%', backgroundColor:'#F9F9F9'}} class="circle-upload fas fa-camera"></i>
            <p style={{marginTop:25}}>Record your video with loom.</p>
             <p>Just press record, and you are ready to record your pitch.</p>
             <Form.Group controlId="formFile" className="mb-3">
