...
export default function BusinessPlan({searchValue,userID,urlID}) {
...
<table className="files-lists table table-striped mt-4">
   ...
   <tbody>
            {files?.filter(file => {
        return file.fileName.toLowerCase().includes(searchValue.toLowerCase()) && file.type.includes("Business")
       }).map((file) => (
