...
function createPitch(video) {
  return (dispatch) => {
    ...
    pitchService.createPitch(video).then(
      (data) => {
        //
  ...
  function request(video) {
    return { type: pitchConstants.ADD_VIDEO_REQUEST, video };
  }
  function success(video) {
    return { type: pitchConstants.ADD_VIDEO_SUCCESS, video };
  }
...
function fetchPitches(id) {
  return (dispatch) => {
    dispatch(request());
    pitchService.fetchPitches(id).then(
      (data) => {
...
function getPitch(id) {
  ...
  function request() {
    return { type: pitchConstants.GET_PITCH_REQUEST };
  }
  function success(video) {
    return { type: pitchConstants.GET_PITCH_SUCCESS, video };
  }
  function failure(error) {
    return { type: pitchConstants.GET_PITCH_FAILURE, error };
  }
}
